// ==UserScript==
// @name         活动报名插件 V4.8.8＋（批量提交+悬浮暂停/继续+示例预览+优化渲染）
// @namespace    https://yourdomain.com
// @version      4.8.8.2
// @description  保留 V4.8.6 UI，批量拉取 & 一次性提交报名，示例预览前5条、累计“报名成功 aa，跳过报名 bb”，仅用悬浮暂停/继续，优化列表渲染。兼容列表页/详情页抽屉逻辑。
...
(function() {
    ...
    if (!isDetail) {
      ...
      d.querySelector('#moduled-submit').onclick = ()=>{
        ...
        Object.assign(window.__moduled_config__, {
          ...
          batchIndex:0, totalBatches:0, successCount:0, skipCount:0
        });
  ...
  function fetchBatchAndSubmit() {
    ...
    cfg.batchIndex++;
    console.log(`🔄 第${cfg.batchIndex}/${cfg.totalBatches||'?'}批，scrollContext="${window.__moduled_scrollContext__}"`);
      ...
      onload(res) {
        ...
        const meetList = batch.filter(it=>{
          ...
          return cfg.mode==='fixed'
            ? sug >= cfg.priceVal
            : ((sug - daily)/daily*100) >= cfg.priceVal;
        });
        cfg.skipCount += (batch.length - meetList.length);

        console.log(`📥 本批拉 ${batch.length} 条，符合 ${meetList.length}，跳过 ${batch.length-meetList.length}`);
            ...
            productList: meetList.map(it=>{
              ...
              return {
                ...
                siteInfoList:[{
                  ...
                  skcList:[{
                    skcId: sk.skcId,
                    skuList:[{skuId:sku.skuId, activityPrice:sku.suggestActivityPrice}]
                  }]
                }]
              };
            })
          ...
          GM_xmlhttpRequest({
            ...
            onload(r2){
              ...
              if(d2.success){
                ...
                console.log(`✅ 第${cfg.batchIndex}批提交成功：${d2.result.successCount}`);
              } else {
              ...
              console.log(`⏳ ${Math.round(delay/1000)}s 后继续`);
              setTimeout(fetchBatchAndSubmit, delay);
            },
            onerror(e2){
              console.error('❌ 提交网络错，5s重试', e2);
              setTimeout(fetchBatchAndSubmit, 5000);
            }
          });
        } else {
          console.log('⚠️ 本批无提交，直接下批');
          fetchBatchAndSubmit();
        }
      },
      onerror(e1){
        console.error('❌ /match 网络错，5s重试', e1);
        setTimeout(fetchBatchAndSubmit, 5000);
      }
    });
  }
  ...
  function renderPreview(meetList) {
    ...
    meetList.slice(0,5).forEach(it=>{
      ...
      const ttl = wds.slice(0,5).join(' ')+(wds.length>5?'...':'');
      ...
      tb.innerHTML += `
        <tr>
          <td><div class="product-cell"><img src="${pic}" /><div class="title" title="${full}">${ttl}</div></div></td>
          <td>${sk.skcId}<br>货号:${sku.extCode||''}</td>
          <td>¥${daily}</td><td>¥${sug}</td><td>${meet}</td><td>${stock}</td><td></td>
        </tr>`;
    });
  }
  ...
  // 你可以在控制台执行 window.__moduled_plugin__() 来手动打开
})();
